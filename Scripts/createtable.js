...
function CreateTableFromServer(jsontable) {
    ...
    for (var i = 1; i < rowcount + 1; i++) {
        ...
        for (var j = 1; j < cellcount + 1; j++) {
            ...
            if ("Button" in table["rows"][i]['cells'][j]) {
                parentcelldiv.appendChild(createbuttonelement(table["rows"][i]['cells'][j]["Button"]['id'], table["rows"][i]['cells'][j]["Button"]['value'], table["rows"][i]['cells'][j]["Button"]['function'], table["rows"][i]['cells'][j]["Button"]['argument']));
            }
...
var createbuttonelement = function (buttonid, buttonval, buttonfunction, arg) {
...
socket.on("comports1", function (data) {
    ...
    var select = document.getElementById("selectcom1");
...
function CreateDeviceTableFromServer(jsontable) {
    var table = JSON.parse(jsontable);
    var rowcount = table.rowcount;
    var cellcount = table.cellcount;
    var nameTable = table.nameTable;
