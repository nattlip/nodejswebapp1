...
exports.doma = require("./Domation");
var jillesrfxcom;
(function (jillesrfxcom) {
    var buffer = [];
    jillesrfxcom.rfxcom = require('rfxcom');
    jillesrfxcom.isinitalised = false;
    jillesrfxcom.dumpHex = function (buffer) {
        //  prefix = prefix || "";
        var jil = 5;
    ...
    jillesrfxcom.test = function (com) {
        jillesrfxcom.rfxtrx = new jillesrfxcom.rfxcom.RfxCom(com, { debug: true });
        //var jil = 5;
        ...
        var serialPort = new SerialPort(com, {
            baudrate: 384000,
            parser: jillesrfxcom.rfxtrx.rfxtrxParser()
        });
        serialPort.open(function (error) {
            if (error) {
                ...
                serialPort.on('data', function (data) {
                    console.log("Received: %s", jillesrfxcom.dumpHex(data));
                    console.log("recieved:  " + jillesrfxcom.dumpHex(data));
                });
                buffer = [0x0D, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00];
                serialPort.write(buffer, function (err, results) {
                    console.log("Sent    : %s", jillesrfxcom.dumpHex(buffer));
                    jillesrfxcom.rfxtrx.delay(500);
    ...
    //end test
    jillesrfxcom.kar = [];
    jillesrfxcom.startrfxcom = function (com) {
        console.log('Hello world');
        // var rfxcom = require('rfxcom');
        jillesrfxcom.rfxtrx = new jillesrfxcom.rfxcom.RfxCom(com, { debug: true });
        jillesrfxcom.rfxtrx.initialise(function () {
            console.log("Device initialised");
            jillesrfxcom.isinitalised = true;
        });
        jillesrfxcom.rfxtrx.on("ready", function (evt) {
            console.log("ready");
            // rollershutterup();
            //for (var i = 0; i < 5; i++)
            //{
        ...
        jillesrfxcom.rfxtrx.on("th1", function (evt) {
            console.log('th1');
        });
        jillesrfxcom.rfxtrx.on("th2", function (evt) {
            console.log('th2');
        });
        jillesrfxcom.rfxtrx.on("th3", function (evt) {
            console.log('th3');
        });
        jillesrfxcom.rfxtrx.on("th4", function (evt) {
            console.log('th4');
        });
        jillesrfxcom.rfxtrx.on("th5", function (evt) {
            console.log('th5');
            console.log("temp:   " + evt.temperature);
        });
        jillesrfxcom.rfxtrx.on("th6", function (evt) {
            console.log('th6');
        ...
        jillesrfxcom.rfxtrx.on("th7", function (evt) {
            console.log('th7');
        });
        jillesrfxcom.rfxtrx.on("th8", function (evt) {
            console.log('th8');
        });
        jillesrfxcom.rfxtrx.on("th9", function (evt) {
            console.log('th9');
        });
        jillesrfxcom.rfxtrx.on("thb1", function (evt) {
            console.log('thb1');
            console.log("id  " + evt.id);
            console.log("temp:   " + evt.temperature);
            console.log("subtype " + evt.subtype);
        ...
        jillesrfxcom.rfxtrx.on("thb2", function (evt) {
            console.log('thb2');
            console.log("id  " + evt.id);
            console.log("temp:   " + evt.temperature);
            console.log("subtype " + evt.subtype);
        ...
        jillesrfxcom.rfxtrx.on('status', function (evt) {
            console.log('status % s % s detected.', evt.subtype, evt.id);
        });
        jillesrfxcom.rfxtrx.on("temp1", function (evt) {
            console.log('temp1');
        });
        jillesrfxcom.rfxtrx.on("temp2", function (evt) {
            console.log('temp2');
        });
        jillesrfxcom.rfxtrx.on("temp3", function (evt) {
            console.log('temp3');
        });
        jillesrfxcom.rfxtrx.on("temp4", function (evt) {
            console.log('temp4');
        });
        jillesrfxcom.rfxtrx.on("temp5", function (evt) {
            console.log('temp5');
        });
        jillesrfxcom.rfxtrx.on("temp6", function (evt) {
            console.log('temp6');
        });
        jillesrfxcom.rfxtrx.on("temp7", function (evt) {
            console.log('temp7');
        });
        jillesrfxcom.rfxtrx.on("temp8", function (evt) {
            console.log('temp8');
        });
        jillesrfxcom.rfxtrx.on("temp9", function (evt) {
            console.log('temp9');
        });
    ...
    jillesrfxcom.listserialports = function (err, ports) {
        var c;
        serialPort.list(function (err, ports) {
            // serialcomarray = ports;
            var jil = 5;
            jillesrfxcom.serialcomarray = ports;
            c = ports;
    ...
    jillesrfxcom.switchon = function () {
        jillesrfxcom.kar[0] = 0x20;
        var currentdevice = jillesrfxcom.getdevice("4");
        var currentunit = jillesrfxcom.getunit("A", "4");
        jillesrfxcom.kar[1] = currentunit;
        jillesrfxcom.kar[2] = 0xFF - jillesrfxcom.kar[1];
        jillesrfxcom.kar[3] = currentdevice;
        jillesrfxcom.kar[4] = 0xFF - jillesrfxcom.kar[3];
        //  send(kar);
        var sw2buffer = [0x07, 0x10, 0x00, 0x01,
            "A".charCodeAt(0), 0x04, 0x01, 0];
        jillesrfxcom.send(sw2buffer);
    };
    jillesrfxcom.switchoff2 = function () {
    ...
    jillesrfxcom.switchdevice = function (json) {
        var jbuffer = [];
        ...
        var currentdevice = jillesrfxcom.getdevice(dc);
        var currentunit = jillesrfxcom.getunit(hc, dc);
    ...
    var switchrollershutter = function (json) {
        ...
        jillesrfxcom.send(shutterbuffer);
    };
    jillesrfxcom.getunit = function (x10char, x10number) {
        var gu;
        gu = jillesrfxcom.xlatunit(x10char);
        if (parseInt(x10number) > 8) {
            gu = gu || 0x04;
        }
        return gu;
    };
    jillesrfxcom.xlatunit = function (x10char) {
        switch (x10char) {
            case "A":
                return 0x60;
                break;
    ...
    jillesrfxcom.getdevice = function (x10number) {
        var temp;
        var dev;
    ...
    jillesrfxcom.send = function (sendbuffer) {
        jillesrfxcom.rfxtrx.serialport.write(sendbuffer, function (err, response, callback) {
            if (jillesrfxcom.rfxtrx.options.debug) {
                console.log("Sent    : %s", jillesrfxcom.rfxtrx.dumpHex(sendbuffer));
            }
            if (callback) {
    ...
    function checkifidexists(even, str) {
        ...
        var devs = exports.device.devicemodule.devices;
        for (var dev in devs) {
            if (parseInt(even.id, 16) !== dev.deviceid) {
                for (var propt in even) {
                    ...
                    if (propt == "temperature" || propt == "humidity" || propt == "barometer") {
                        ...
                        exports.sqdb.dbase.adddevicetodbase(thisdevicejson);
                    }
                }
            }
        }
    }
})(jillesrfxcom = exports.jillesrfxcom || (exports.jillesrfxcom = {}));
;
//# sourceMappingURL=jillesrfxcom.js.map