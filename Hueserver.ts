...
import obs = require("./observe")
import events = require('events');
var sys = require("sys");
import rfxcom = require("./jillesrfxcom");
import device = require("./device");
import createtable = require("./createtableserverside")
import sqdb = require("./sqlite");

  export  module HueServer
{
...
// start of dbase management
      ...
      var bridg: datastore.datafrombridge.Bridgeclass;
...
sqdb.dbase.checkdbase();
    ...
    function processRequest(request: http.ServerRequest, response)
    {
        "use strict";
  ...
  export   var socketclient;
          ...
          sio.sockets.on('connection', function (socket)
          {
             ...
             socketclient = socket;
              ...
              socket.on('clearpage', function (data)
              {
                  ...
                  writetopage.writeHead(1000, { 'Content-Type': 'text/html' });
                  writetopage.write('<!DOCTYPE html><html ><head>');
                  writetopage.write('<meta charset="utf-8">');
                  writetopage.write('<title>' + 'Hue Node!' + '</title>');
                  writetopage.write('</head><body>');
                  writetopage.write('</body></html>');
                  writetopage.end();
              ...
              socket.on('createcanvas', function (data)
              {
                  ...
                  socketclient.emit('canvascreatepage', bjson);
            ...
            //  to do guid//
                                  ...
                                  var a = function ()             
                  {
                      ...
                      serialPort.list(function (err, ports)
                      {
                          ...
                          scpjson = JSON.stringify(portseffective)
                              var jil = 5;
                          socketclient.emit("comports", scpjson);
                      });
                  }
              ...
              socket.on('getmasters', function (datajsonstring)
              {
                  ...
                  socketclient.emit("masters", json);
              }
                  )
            ...
            device.devicemodule.Devicefunctions.selectenddevice(datajsonstring);
              ...
              socket.on("createdevicetable", function ()
              {
                  ...
                  createtable.createtableserverside.createdevicetablewithjson(socketclient,device.devicemodule.devices);
              });
    ...
    stdin.addListener("data", function (d)
    {
        ...
        socketclient.emit('newPage', req.jil.cache)
...
function fillrowstable()
    ...
    var callback = function (err, results)
    {
            ...
            if (datastore.datafrombridge.bridge[0] !== "{")
        {
            ...
            {
                socketclient.emit('newPage', JSON.stringify(datastore.datafrombridge.bridge));
            };
        };
    };
   ...
   findipbridge(callback);
    ...
    function passlightstopage(lights)
    {
        ...
        socketclient.emit('lights', JSON.stringify(lights));
   ...
   socketclient.emit('topMenu', b);
    ...
    function createpage()
    {
        ...
        socketclient.emit('createPage')

}


    export function updatelights(data): void
    {



        socketclient.emit("UpdateLights", data)
